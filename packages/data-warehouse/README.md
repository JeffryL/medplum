...
- Idempotent export using time windows: **`--start-window` is required**; **`--end-window` is optional**. With both flags, the window is half-open `[startWindow, endWindow)` on `lastUpdated`. If `--end-window` is omitted, all rows with `lastUpdated >= startWindow` are included (no upper bound).
...
To export everything from a start time onward (no `--end-window`):
...
Omit `--end-window` in the `export` arguments if you want an unbounded upper range (same as the CLI above).
