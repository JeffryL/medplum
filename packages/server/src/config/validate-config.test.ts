...
import { assertEnabledDataWarehouseComplete, validateDataWarehouseConfig } from './validate-config';
...
describe('assertEnabledDataWarehouseComplete', () => {
  it('reuses the same rules as validateDataWarehouseConfig for enabled blocks', () => {
    expect(() => assertEnabledDataWarehouseComplete({ cron: '0 * * * *', sink: 's3tables' })).toThrow(
