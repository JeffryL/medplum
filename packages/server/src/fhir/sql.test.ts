...
describe('SqlBuilder', () => {
  ...
  describe('SelectQuery', () => {
    ...
    test('Select where not equals expression value', () => {
      const sql = new SqlBuilder();
      new SelectQuery('MyTable').column('id').where('name', '!=', new Constant(`''`)).buildSql(sql);
    ...
    test('Insert into select executes query', async () => {
      const db = {
